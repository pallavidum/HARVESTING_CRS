...
export class CreateModelComponent implements OnInit {
    ...
    if (fi.files) {
      ...
      this.uploadSubscription = this.modelService
        ...
        .subscribe(res => {
          ...
          // TODO this loop should always run only once
          // if this loop runs multiple times it must be rewritten
          // because at the end of this loop the next page is automatically loaded
          for (let i = 0; i < this.uploadedFiles.length; i++) {
            this.modelService.featureType(`${environment.apiHost}/Uploads/${fileData[i].fileName}`).subscribe(result => {
            console.log(result);
            const columns: Column[] = fileData[i].cols.map((col, index) => <Column>{
              name: col,
              isDeleted: false,
              //isNumeric: fileData[i].featureTypes[index],
              isNumeric: result[col],
  ...
  // TODO takes a long time to load, may want to move elsewhere
  combineData() {
    console.log(this.modelConfiguration.dataSets[0].csvPath);
    this.modelService.combineUserAie(this.modelConfiguration.dataSets[0].csvPath).subscribe(res => {
      console.log(res);
    });
  }
  fileUploadChange(e) {
